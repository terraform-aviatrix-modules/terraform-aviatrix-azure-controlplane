...
resource "azurerm_marketplace_agreement" "aviatrix_controller_mp_agreement" {
  ...
  offer     = jsondecode(data.http.controller_image_info.response_body)["g3"]["amd64"]["Azure ARM"]["offer"]
  plan      = jsondecode(data.http.controller_image_info.response_body)["g3"]["amd64"]["Azure ARM"]["sku"]
}
