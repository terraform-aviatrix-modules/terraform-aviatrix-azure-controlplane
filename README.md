...
| <a name="input_controller_os_disk_size_gb"></a> [controller\_os\_disk\_size\_gb](#input\_controller\_os\_disk\_size\_gb) | Size of the controller OS disk in GB. Defaults to 128 (P10, 500 IOPS) for controller 8.0 and above, which require that minimum, and to the source image size for earlier versions. At larger scale use 512 (P20, 2300 IOPS) or 1024 (P30, 5000 IOPS) for more IOPS. Azure bills a premium disk at the next tier up, so sizes between tiers provide no extra IOPS. | `number` | `null` | no |
| <a name="input_controller_os_disk_storage_account_type"></a> [controller\_os\_disk\_storage\_account\_type](#input\_controller\_os\_disk\_storage\_account\_type) | Storage account type for the controller OS disk. Defaults to Premium\_LRS for controller 8.0 and above, which require a premium disk to meet the 500 IOPS minimum, and Standard\_LRS for earlier versions. | `string` | `null` | no |
